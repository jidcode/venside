  ...
  return (
    <>
      <Sheet open={isOpen} onOpenChange={setIsOpen}>
        ...
        <SheetTitle className="sr-only">Add Customer</SheetTitle>

        <SheetContent className="flex flex-col bg-primary border-none h-full min-w-full md:min-w-1/3">
          <AddCustomerForm closeSheet={handleClose} />
        </SheetContent>
      </Sheet>
    </>
  );
...
function AddCustomerForm({ closeSheet }: { closeSheet: () => void }) {
  ...
  const action: SubmitHandler<CustomerRequest> = async (formData) => {
