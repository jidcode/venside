...
export default function DeleteSaleDialog({ sale }: { sale: SaleState }) {
  const router = useRouter();

  const [isDeleting, setIsDeleting] = useState(false);
  const [open, setOpen] = useState(false);
  const { deleteSale } = useSaleService();
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      errorToast("Failed to delete sale. Please try again.");
    } finally {
      setIsDeleting(false);
    }
  };
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Are you sure you want to delete sale #{sale.saleNumber}? This action
